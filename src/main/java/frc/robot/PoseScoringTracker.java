...
public class PoseScoringTracker {
    ...
    private static boolean[] probablyScoredPoses = new boolean[SCORING_POSES_COUNT * SCORING_LEVELS_COUNT];

    /**
     * Marks the closest fudged scoring pose as scored at the current scoring level.
     * Called when the robot successfully scores a piece.
     */
    public static void markPoseScored() {
        IndexedValue<Pose2d> closestPose = getClosestFudgedScoringPose(Robot.getInstance().getEstimatedPose());
        if (closestPose == null) {
            return;
        }
    ...
    /**
     * Calculates a score for how well the robot is aligned to a given pose.
     * Lower score = better alignment (closer translation + better rotation).
     */
    private static double score(Pose2d targetPose, Pose2d currentPose) {
        double translationDistance = targetPose.getTranslation().getDistance(currentPose.getTranslation());
        double rotationDifference = Math.abs((targetPose.getRotation().minus(currentPose.getRotation())).getRadians());
    ...
    /**
     * Finds the closest coral/reef scoring pose that hasn't been used yet.
     * Considers "badness" factor for already scored positions.
     */
    public static IndexedValue<Pose2d> getClosestFudgedScoringPose(Pose2d estimatedPose) {
        List<Pose2d> poses = Robot.getInstance().isOnRedSide() ? Constants.Field.redScoringPoses
        ...
        Map.Entry<Integer, Pose2d> result = poses.stream()
                .filter(pose -> pose.getTranslation()
                        .getDistance(estimatedPose.getTranslation()) < Constants.Extra.MAX_NODE_DISTANCE)
                .collect(Collectors.toMap(
                        pose -> poses.indexOf(pose),
                        pose -> pose))
                .entrySet()
                .stream()
                .min((entry1, entry2) -> {
                    ...
                    double fudge1 = wasPoseScored(idx1)
                            ? Constants.Extra.ALREADY_SCORED_BADNESS
                            : 0.0;
                    double fudge2 = wasPoseScored(idx2)
                            ? Constants.Extra.ALREADY_SCORED_BADNESS
                            : 0.0;
    ...
    public static boolean atGoodScoringDistance() {
        if (!Robot.getInstance().isOnRedSide()) {
            // Blue alliance
            return Robot.getInstance().getEstimatedPose().getX() > Constants.Field.BLUE_BARGE_SCORING_X
                    - BARGE_SCORING_POSITION_TOLERANCE
                    && Robot.getInstance().getEstimatedPose().getX() < Constants.Field.BLUE_BARGE_SCORING_X
                            + BARGE_SCORING_POSITION_TOLERANCE;
        } else {
            // Red alliance
            return Robot.getInstance().getEstimatedPose().getX() > Constants.Field.RED_BARGE_SCORING_X
                    - BARGE_SCORING_POSITION_TOLERANCE
                    && Robot.getInstance().getEstimatedPose().getX() < Constants.Field.RED_BARGE_SCORING_X
                            + BARGE_SCORING_POSITION_TOLERANCE;
        }
    }
