...
public class Arm extends SubsystemBase {
    ...
    public Side getSideCloserToReef() {
        Rotation2d directionTowardReefCenter = Utils.mirrorIfRed(
                Constants.Field.BLUE_REEF_CENTER)
    ...
    public boolean atSafeReefDistance() {
        return Robot.getInstance().getEstimatedPose().getTranslation()
                .getDistance(
                        Utils.mirrorIfRed(
                                Constants.Field.BLUE_REEF_CENTER)) > Constants.Arm.SAFE_DISTANCE_FROM_REEF_CENTER;
    }

    public boolean atSafePlacementDistance() {
        return Robot.getInstance().getEstimatedPose().getTranslation()
                .getDistance(
                        Utils.mirrorIfRed(Constants.Field.BLUE_REEF_CENTER)) > Constants.Arm.SAFE_PLACEMENT_DISTANCE;
    }
    ...
    public double positionFromAngle(double angle, boolean respectReef) {
        ...
        if (positions.size() == 1) {
            ...
            switch (closeSide) {
                case Neither:
                    p = positions.stream()
                            .min((a, b) -> Double.compare(Math.abs(a - actualArmPosition),
                                    Math.abs(b - actualArmPosition)))
    ...
    public double getDesiredPosition() {
        ...
        double answer = positionFromAngle(pivotState.getDesiredAngle(),
    ...
    public void initSendable(SendableBuilder builder) {
        builder.addDoubleProperty("Arm offset (deg)",
                () -> Math.toDegrees(armOffsetRadians), null);
        builder.addDoubleProperty("Arm position (deg)",
                () -> Math.toDegrees(getPosition()), null);
        builder.addDoubleProperty("Raw Encoder",
                () -> absoluteEncoder.getPosition().getValueAsDouble(), null);
        builder.addBooleanProperty("Has object? ",
                () -> hasObject, null);
        builder.addDoubleProperty("Desired position (deg)",
                () -> Math.toDegrees(getDesiredPosition()), null);
        ...
        builder.addBooleanProperty("At setpoint?",
                () -> isAtSetpoint(), null);
        builder.addDoubleProperty("roller current",
                () -> statorCurrentSignal.getValueAsDouble(), null);
