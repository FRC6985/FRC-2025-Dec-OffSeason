...
public class Intake extends SubsystemBase {
    ...
    private Intake() {
        ...
        centeringMotor.getConfigurator().apply(
                new MotorOutputConfigs().withInverted(InvertedValue.Clockwise_Positive));
    ...
    public boolean hasCoral() {
        return !linebreak.get() || Controls.getInstance().operatorController.getHID().getTouchpadButton();
    }
    ...
    private boolean isUnsafeToGoUp() {
        ...
        double elevatorToArmLimit = Math.PI
    ...
    public void periodic() {
        ...
        pivotMotor.setControl(
                new MotionMagicVoltage(effectivePivot.angleSetpoint / (2 * Math.PI)));
    ...
    public void initSendable(SendableBuilder builder) {
        builder.addDoubleProperty("Intake angle",
                () -> Math.toDegrees(getAngle()), null);
        builder.addDoubleProperty("Intake setpoint",
                () -> Math.toDegrees(getEffectivePivotState().angleSetpoint), null);
        builder.addBooleanProperty("at setpoint?",
                this::isAtSetpoint, null);
        builder.addBooleanProperty("Intake have coral",
                this::hasCoral, null);
        builder.addStringProperty("Effective intake pivot state",
                () -> getEffectivePivotState().toString(), null);
        builder.addStringProperty("Underlying intake pivot state",
                () -> realPivotState.toString(), null);
        builder.addBooleanProperty("Is Zeroed?",
                () -> isZeroed, null);
        Utils.addClosedLoopProperties("Intake Pivot", pivotMotor, builder);
        builder.addBooleanProperty("unsafe for intake to go up?",
                this::isUnsafeToGoUp, null);
    }
}