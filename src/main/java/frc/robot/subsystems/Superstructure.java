...
public class Superstructure extends SubsystemBase {
  ...
  private void initializeTransitions() {
        ...
        new Transition(
            ...
            () ->
                Elevator.getInstance().isAtSetpoint()
                    && Arm.getInstance().isAtSetpoint()
                    && Intake.getInstance().isAtSetpoint()));
    transitions.add(new Transition(State.Handoff, State.Rest, () -> Arm.getInstance().isArmStuck));
    transitions.add(
        new Transition(
            State.Rest,
            State.PreScore,
    ...
    transitions.add(
        new Transition(State.GetAlgae, State.PostGetAlgae, () -> Arm.getInstance().isArmStuck));
    transitions.add(
        new Transition(
            State.PostGetAlgae,
            State.AlgaeRest,
    ...
    transitions.add(
        new Transition(State.AlgaeRest, State.AlgaeExit, () -> !Arm.getInstance().isArmStuck));
    transitions.add(
        new Transition(
            State.AlgaeExit,
            State.Rest,
            ...
            () ->
                !inputs.wantPopsiclePickup
                    || (Robot.getInstance().isAutonomous()
                        && stateTimer.hasElapsed(POPSICLE_DELAY))));
    transitions.add(
  ...
  private Transition[] scoringTransitions(
      ScoringLevel level, State prepare, State start, State place, State after) {
  ...
  public void periodic() {
    if (!Elevator.getInstance().isZeroed
